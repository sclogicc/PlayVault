...
export default function Timeline(): React.ReactElement {
  ...
  return (
    <div className="content-canvas min-h-full space-y-8 bg-[var(--pv-void)] px-7 py-8 transition-colors duration-300 sm:px-10 sm:py-9 lg:px-12">
      <header className="flex flex-wrap items-end justify-between gap-6 border-b border-white/[0.075] pb-6">
        <div>
          ...
          <p className="mt-3 max-w-2xl text-[13px] leading-6 text-archive-500">按月份回看真正游玩过的游戏，而不是只浏览一张静态的游戏清单。</p>
        </div>
        <div className="flex gap-1 border border-white/[0.075] bg-white/[0.025] p-1" aria-label="时间范围">
          {RANGE_OPTIONS.map((option) => <button key={option} type="button" aria-pressed={days === option} onClick={() => setDays(option)} className={`min-h-7 px-2.5 text-[11px] font-medium transition-[background-color,color,transform] duration-200 ${days === option ? 'bg-[color:color-mix(in_srgb,var(--pv-accent)_18%,transparent)] text-[var(--pv-accent-strong)]' : 'text-archive-500 hover:bg-white/[0.055] hover:text-archive-200'}`}>近 {option} 天</button>)}
        </div>
      </header>

      <section className="grid grid-cols-1 gap-px border border-white/[0.065] bg-white/[0.065] sm:grid-cols-3" aria-label="时间范围汇总">
...
function DayLog({ date, sessions, gameById }: { date: string; sessions: SessionWithGame[]; gameById: Map<number, GameWithStats> }): React.ReactElement {
  ...
  return (
    <article className="border border-white/[0.065] bg-white/[0.02] transition-[border-color,background-color] duration-300 hover:border-white/[0.11] hover:bg-white/[0.027]">
